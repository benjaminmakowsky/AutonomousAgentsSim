...
void Paint_HUD_values(void) {
  ...
  static char fuelbuf[32];
  static char bugbuf[50];
  ...
  sprintf(fuelbuf, "fuel : %.2f", fuel);
  sprintf(bugbuf, "%s", bugstring);
  ...
  //fov
  y += lineSep;
  rd.drawString(dpy, drawPixmap, gameGC, x, y, fuelbuf, lenFuel);

  //debug string
  y += lineSep;
  rd.drawString(dpy, drawPixmap, gameGC, x, y, bugbuf, lenDebug);
