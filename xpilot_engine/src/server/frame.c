 ...
 */
	static inline bool
clpos_inview (click_visibility_t * v, clpos_t pos)
{
...
num_++;
	...
	static void
fastshot_store (int cx, int cy, int color, int offset)
{
	int xf = CLICK_TO_PIXEL (cx), yf = CLICK_TO_PIXEL (cy);
...
#undef max_
	...
	static void
debris_store (int cx, int cy, int color)
{
	int xf = CLICK_TO_PIXEL (cx), yf = CLICK_TO_PIXEL (cy);
...
#undef max_
	...
	static void
fastshot_end (connection_t * conn)
{
	int i;
	...
	for (i = 0; i < DEBRIS_TYPES * 2; i++)
	{
		if (fastshot_num[i] != 0)
		{
			Send_fastshot (conn, i,
					(unsigned char *) fastshot_ptr[i], fastshot_num[i]);
			fastshot_num[i] = 0;
		}
	}
}
...
debris_end (connection_t * conn)
{
	...
	for (i = 0; i < DEBRIS_TYPES; i++)
	{
		...
		{
			Send_debris (conn, i,
					(unsigned char *) debris_ptr[i], debris_num[i]);
			debris_num[i] = 0;
		}
	}
}
	...
	static void
Frame_radar_buffer_reset (void)
{
	num_radar = 0;
}

	static void
Frame_radar_buffer_add (clpos_t pos, int s)
{
	radar_t *p;
	...
	static void
Frame_radar_buffer_send (connection_t * conn, player_t * pl)
{
	int i, dest, tmp;
	radar_t *p;
...
Frame_radar_buffer_free (void)
{
	...
	static int
Frame_status (connection_t * conn, player_t * pl)
{
	...
	if (BIT (pl->lock.tagged, LOCK_PLAYER) && Player_uses_compass (pl))
	{
		...
		if ((!BIT (world->rules->mode, LIMITED_VISIBILITY)
				...
				&& (options.playersOnRadar
					|| clpos_inview (&cv, lock_pl->pos)) && pl->lock.distance != 0)
		{
			double a;
	...
	n = Send_self (conn,
			...
			lock_dir,
			showautopilot,
			Player_by_id (Get_player_id (conn))->pl_old_status, modsstr);
	if (n <= 0)
		return 0;

	if (Player_uses_emergency_thrust (pl))
		Send_thrusttime (conn,
				(int) pl->emergency_thrust_left, EMERGENCY_THRUST_TIME);
	if (BIT (pl->used, HAS_EMERGENCY_SHIELD))
		Send_shieldtime (conn,
				(int) pl->emergency_shield_left, EMERGENCY_SHIELD_TIME);
	if (Player_is_self_destructing (pl))
		Send_destruct (conn, (int) pl->self_destruct_count);
	if (Player_is_phasing (pl))
		Send_phasingtime (conn, (int) pl->phasing_left, PHASING_TIME);
	if (ShutdownServer != -1)
	...
	static void
Frame_map (connection_t * conn, player_t * pl)
{
...
Frame_shuffle (void)
{
	...
	static void
Frame_shots (connection_t * conn, player_t * pl)
{
	clpos_t pos;
		...
		else
		{
			...
			else
			{
				pos.cx = (click_t) (pos.cx
						-
						tcos (pulse->pulse_dir) * pulse->pulse_len *
						CLICK);
				pos.cy =
					(click_t) (pos.cy -
							tsin (pulse->pulse_dir) * pulse->pulse_len *
							CLICK);
		...
		switch (shot->type)
		{
			...
			case OBJ_DEBRIS:
				...
				debris_store (shot->pos.cx - cv.unrealWorld.cx,
						shot->pos.cy - cv.unrealWorld.cy, color);
				break;
			...
			case OBJ_CANNON_SHOT:
				...
				fastshot_store (shot->pos.cx - cv.unrealWorld.cx,
						shot->pos.cy - cv.unrealWorld.cy, color, teamshot);
				break;
	...
	static void
Frame_ships (connection_t * conn, player_t * pl)
{
	int i, k;
	...
	for (i = 0; i < Num_transporters (); i++)
	{
		...
		if (clpos_inview (&cv, victim->pos) || clpos_inview (&cv, pos))
	...
	for (k = 0; k < num_player_shuffle; k++)
	{
		...
		if (Player_is_paused (pl_i) || Player_is_appearing (pl_i))
		{
			...
			if (Player_is_paused (pl_i))
				Send_paused (conn, pl_i->home_base->pos, (int) pl_i->pause_count);
			else
	...
	static void
Frame_radar (connection_t * conn, player_t * pl)
{
	...
	if (options.playersOnRadar
			|| BIT (world->rules->mode, TEAM_PLAY)
			|| NumPseudoPlayers > 0 || NumAlliances > 0)
	{
		for (k = 0; k < num_player_shuffle; k++)
		{
			player_t *pl_i;
			...
			if (Players_are_teammates (pl_i, pl)
					|| Players_are_allies (pl, pl_i) || Player_owns_tank (pl, pl_i))
				size |= 0x80;
			Frame_radar_buffer_add (pos, size);
		}
	}
	...
	static void
Frame_lose_item_state (player_t * pl)
{
	...
	static void
Frame_parameters (connection_t * conn, player_t * pl)
{
...
Frame_update (void)
{
	...
	for (i = 0; i < spectatorStart + NumSpectators; i++)
	{
		...
		if ((Player_is_paused (pl)
					...
					|| Player_is_dead (pl)) && pl->rectype != 2)
		{
			/*
...
Set_message (const char *message)
{
	...
	void
Set_player_message (player_t * pl, const char *message)
{
	int i;
	const char *msg;
	char tmp[MSG_LEN];
...
Set_message_f (const char *fmt, ...)
{
	...
	void
Set_player_message_f (player_t * pl, const char *fmt, ...)
{
	size_t len;
	static char msg[2 * MSG_LEN];
	va_list ap;
	...
	if ((len = strlen (msg)) >= MSG_LEN)
	{
		warn ("Set_player_message_f: Max len exceeded (%d,\"%s\")", len, msg);
		msg[MSG_LEN - 1] = '\0';
		assert (strlen (msg) < MSG_LEN);
	}
