...
{
	double angleDiff = 360.0 / ( pl->numInRole );
	double ourAngle = angleDiff * pl->idxInRole; 
	int dir;

	if( !strcmp( pl->role, "reaction_force" ) ){
		strncpy( pl->rallypoint.role, "reaction_force", sizeof("reaction_force") );

		//should form a small circle around the objective
		int radius = 30;  // TODO: Change this assumption
		pl->rallypoint.pos.cx = pl->objpoint.pos.cx + radius * cos( angleDiff );
		pl->rallypoint.pos.cy = pl->objpoint.pos.cy + radius * sin( angleDiff );

	}
	else if( !strcmp( pl->role, "stationary_defend" ) ){
		strncpy( pl->rallypoint.role, "stationary_defend", sizeof("stationary_defend") );

		//should form a large circle around the objective
		int radius = 60;  // TODO: Change this assumption
		pl->rallypoint.pos.cx = pl->objpoint.pos.cx + radius * cos( angleDiff );
		pl->rallypoint.pos.cy = pl->objpoint.pos.cy + radius * sin( angleDiff );

	}

	double a = findDir(pl->rallypoint.pos.cx, pl->rallypoint.pos.cy);
	dir = MOD2((int) (a + 0.5 ), RES );
	dir = ((dir + RES/8) / (RES/4)) * (RES/4);
	pl->rallypoint.dir = MOD2(dir,RES);
	pl->rallypoint.isset = true;
...
void Go_home(player_t *pl)
{
	...
	if( pl->rallypoint.pos.cx == 0 || pl->rallypoint.pos.cy == 0 ){
...
void Player_death_reset(player_t *pl, bool add_rank_death)
{
	...
	else if( !strcmp( pl->ship->name, "tank" ) ){

		pl->emptymass = pl->mass = 50; 
	}
	else if( !strcmp( pl->ship->name, "infantry" ) ){
