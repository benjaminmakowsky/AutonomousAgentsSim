...
JNIEXPORT void JNICALL Java_javaAI_reprogram(JNIEnv * env, jobject jobj) {
    Keyboard_button_pressed(XK_quoteleft);
	Keyboard_button_released(XK_quoteleft);
}
...
JNIEXPORT jstring JNICALL Java_javaAI_scanMsg(JNIEnv * env, jobject jobj, int id) {
    if (id<MAX_MSGS) return (*env)->NewStringUTF(env,TalkMsg[id]);
    return "";
}
...
JNIEXPORT double JNICALL Java_javaAI_selfTrackingRad(JNIEnv * env, jobject jobj) {  //returns the player's tracking in radians	-JNE
    double ans = atan((double)Java_javaAI_selfVelY(env,jobj)/(double)Java_javaAI_selfVelX(env,jobj));       //calculate tracking
    if (Java_javaAI_selfVelY(env,jobj) >= 0 && Java_javaAI_selfVelX(env,jobj) < 0) {        //format the tracking properly
        ans += PI_AI;
    }
    else if (Java_javaAI_selfVelY(env,jobj) < 0 && Java_javaAI_selfVelX(env,jobj) < 0) {
        //ans = -ans;
        ans += PI_AI;
    }
    else if (Java_javaAI_selfVelY(env,jobj) < 0 && Java_javaAI_selfVelX(env,jobj) >= 0) {
...
JNIEXPORT double JNICALL Java_javaAI_selfTrackingDeg(JNIEnv * env, jobject jobj) {  //returns the player's tracking in degrees -JNE
    return (double)Java_javaAI_selfTrackingRad(env,jobj)*180/PI_AI	;
}
...
JNIEXPORT jstring JNICALL Java_javaAI_hudName(JNIEnv * env, jobject jobj) {         //if the HUD is displaying a name, return it	-JNE
    int i;
...
JNIEXPORT jstring JNICALL Java_javaAI_hudScore(JNIEnv * env, jobject jobj) {        //if the HUD is displaying a score, return it	-JNE
    int i;
    for (i = 0; i < MAX_SCORE_OBJECTS; i++) {
        if (score_objects[i].hud_msg_len>0) {
...
JNIEXPORT double JNICALL Java_javaAI_hudTimeLeft(JNIEnv * env, jobject jobj) {      //returns how much time the HUD will keep displaying a score for, in seconds	-JNE
    int i;
    for (i=0;i<MAX_SCORE_OBJECTS; i++) {
        if (score_objects[i].hud_msg_len>0) {
            return score_objects[i].life_time;
        }
    }
    return 0;
}
...
JNIEXPORT jstring JNICALL Java_javaAI_selfName(JNIEnv * env, jobject jobj) {
    if (self != NULL) return (*env)->NewStringUTF(env,self->nick_name);
}
...
JNIEXPORT jstring JNICALL Java_javaAI_enemyNameId(JNIEnv * env, jobject jobj, int id) {       //return the name of the enemy with the corresponding id	-JNE
    ...
    for (i=0;i<num_others;i++) {    //look for the other with the id that was passed in
        if (Others[i].id==id) {
            return (*env)->NewStringUTF(env,Others[i].nick_name); //make a jstring out of nick_name
        }
    }
    return NULL;
}
...
JNIEXPORT int JNICALL Java_javaAI_wallFeeler(JNIEnv * env, jobject jobj, int dist, int angle, int flag_wf, int flag_wd) {
    double a = angle * PI_AI / 180.0;
    int x = selfPos.x + cos(a)*dist;
    int y = selfPos.y + sin(a)*dist;
    int ret = Java_javaAI_wallBetween(env, jobj, selfPos.x, selfPos.y, x, y, flag_wf, flag_wd);
    if (ret == -1) return dist; //Returns the distance of the feeler if no wall is felt - JTO
    return ret;
}
//wallFeeler that uses radians! -EGG
JNIEXPORT int JNICALL Java_javaAI_wallFeelerRad(JNIEnv * env, jobject jobj, int dist, double a, int flag_wf, int flag_wd) {
    int x = selfPos.x + cos(a)*dist;
    int y = selfPos.y + sin(a)*dist;
    int ret =  Java_javaAI_wallBetween(env, jobj, selfPos.x, selfPos.y, x, y, flag_wf, flag_wd);
    if (ret == -1) return dist; //Returns the distance of the feeler if no wall is felt - JTO
...
JNIEXPORT int JNICALL Java_javaAI_wallBetween(JNIEnv * env, jobject jobj, int x1, int y1, int x2, int y2, int flag_wf, int flag_wd) {
...
double selfTrackingRad() {
    double ans = atan((double)selfVel.y/(double)selfVel.x);
    if (selfVel.y >= 0 && selfVel.x < 0)
        ans += 3.141592653589793;
    else if (selfVel.y < 0 && selfVel.x < 0)
	ans += 3.141592653589793;
    else if (selfVel.y < 0 && selfVel.x >= 0)
...
//END inject -EGG
//THE JNI TO START XPI_AILOT CLIENT. Called as AI.start(int,String[]). -EGG
