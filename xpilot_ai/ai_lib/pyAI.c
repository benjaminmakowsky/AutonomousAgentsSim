...
int selfVelX() {    //returns the player's x velocity //DO NOT CHANGE, NEEDED IN ORDER FOR selfTrackingRad to work -JRA
        return selfVel.x;
}
static PyObject* py_selfVelX(PyObject* pySelf, PyObject* args) {    //returns the player's x velocity
        return Py_BuildValue("i",selfVel.x);
}
int selfVelY() {    //returns the player's y velocity //DO NOT CHANGE, NEEDED IN ORDER FOR selfTrackingRad to work -JRA
        return selfVel.y;
}
static PyObject* py_selfVelY(PyObject* pySelf, PyObject* args) {    //returns the player's y velocity
        return Py_BuildValue("i",selfVel.y);
}
...
static PyObject* py_selfSpeed(PyObject* pySelf, PyObject* args) {   //returns speed of the player's ship
        return Py_BuildValue("d",sqrt(pow(selfVel.x,2)+pow(selfVel.y,2)));
}
...
static PyObject* py_hudName(PyObject* pySelf, PyObject* args) {         //if the HUD is displaying a name, return it	-JNE
        int i;
...
static PyObject* py_hudTimeLeft(PyObject* pySelf, PyObject* args) {      //returns how much time the HUD will keep displaying a score for, in seconds	-JNE
        ...
        for (i=0;i<MAX_SCORE_OBJECTS; i++) {
                if (score_objects[i].hud_msg_len>0) {
                        return Py_BuildValue("d",score_objects[i].life_time);
                }
        }
...
static PyObject* py_selfName(PyObject* pySelf, PyObject* args) {
    if (self != NULL) return Py_BuildValue("s",self->nick_name);
}
...
static PyObject* py_enemyNameId(PyObject* pySelf, PyObject* args) {       //return the name of the enemy with the corresponding id	-JNE
    ...
    for (i=0;i<num_others;i++) {    //look for the other with the id that was passed in
        if (Others[i].id==id) {
            return Py_BuildValue("s",Others[i].nick_name);
        }
    }
...
static PyObject* py_wallFeeler(PyObject* pySelf, PyObject* args) {
    int dist;
    int angle;
    int flag_wf;
    int flag_wd;
    if (!PyArg_ParseTuple(args, "iiii", &dist, &angle, &flag_wf, &flag_wd)){
        PyErr_SetString(PyExc_TypeError, "invalid parameter");
        return NULL;
    }
...
static PyObject* py_wallFeelerRad(PyObject* pySelf, PyObject* args) {
    int dist; 
    double a; 
    int flag_wf;
    int flag_wd;
    if (!PyArg_ParseTuple(args, "idii", &dist, &a, &flag_wf, &flag_wd)){
        PyErr_SetString(PyExc_TypeError, "invalid parameter");
        return NULL;
    }
    int x = selfPos.x + cos(a)*dist;
...
int wallBetween(int x1, int y1, int x2, int y2, int flag_wf, int flag_wd) { //DO NOT CHANGE, NEEDED IN ORDER FOR WallFeeler & WallFeelerRad to work -JRA
...
static PyObject* py_wallBetween(PyObject* pySelf, PyObject* args) {
    ...
    int flag_wf;
    int flag_wd;
    if (!PyArg_ParseTuple(args, "iiiiii", &x1, &y1, &x2, &y2, &flag_wf, &flag_wd)){
        PyErr_SetString(PyExc_TypeError, "invalid parameter");
        return NULL;
    }
    int i,dx,dy,sdx,sdy,dxabs,dyabs,x,y,px,py,ret;
...
int Wall_here(int x_here, int y_here, int flag_wf, int flag_wd) { //DO NOT CHANGE -JRA
...
static PyObject* py_aimdir(PyObject* pySelf, PyObject* args) {
	...
	if ((Sy + Svy * time2) <= (selfPos.y + sin(AI_radian(selfTrackingDeg())) * selfSpeed() * time2)) return Py_BuildValue("i",360 - (int)degs2);
	else return Py_BuildValue("i",-1);
    }
    else if (time2 < 0) {
	if ((Sy + Svy * time1) >= (selfPos.y + sin(AI_radian(selfTrackingDeg())) * selfSpeed() * time1)) return Py_BuildValue("i",(int)degs1);
	else return Py_BuildValue("i",-1);
    }
    else {
	if ((Sy + Svy * time1) >= (selfPos.y + sin(AI_radian(selfTrackingDeg())) * selfSpeed() * time1)) return Py_BuildValue("i",(int)degs1);
	else if ((Sy + Svy * time2) < (selfPos.y + sin(AI_radian(selfTrackingDeg())) * selfSpeed() * time2)) return Py_BuildValue("i",360 - (int)degs2);
...
static PyMethodDef libpyAI_methods[] = {
    ...
    {"hudName",py_hudName,METH_NOARGS,"Returns the Name on the HUD"},
    {"hudScore",py_hudScore,METH_NOARGS,"Returns the Score on the HUD"},
    {"hudTimeLeft",py_hudTimeLeft,METH_NOARGS,"Returns the Time Left on the HUD"},
